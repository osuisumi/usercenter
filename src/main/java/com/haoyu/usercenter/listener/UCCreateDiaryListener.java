...
@Component
public class UCCreateDiaryListener implements ApplicationListener<CreateDiaryEvent>{
	@Resource
	...
	@Override
	public void onApplicationEvent(CreateDiaryEvent event) {
		Diary diary = (Diary) event.getSource();
		Dynamic dynamic = DynamicFactory.createDynamic(diary);
		if(dynamic!=null){
			...
			if(response.isSuccess()){
				...
				if(CollectionUtils.isNotEmpty(myfans)){
					...
					message.setContent("您关注的用户'"+ThreadContext.getUser().getRealName()+"'添加了日志:"+diary.getTitle());
